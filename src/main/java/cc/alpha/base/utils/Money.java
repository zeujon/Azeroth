...
public class Money {
	...
	public static String getChnmoney(String strNum) {
		...
		// 定义大写货币单位的排列数组
		String as1[] = { "仟", "佰", "拾", "万", "仟", "佰", "拾", "亿", "仟", "佰", "拾",
				"万", "仟", "佰", "拾", "元", "角", "分" };
		...
		if (d == 0.0D) {
			...
			for (i = 0; i < i1; i++) {
				...
				if (as2[i].compareTo("零") == 0) {
					...
					if (as2[i + 1] != null) {
						// 判断零后的字符是否为万，亿，元三个字符
						if (as2[i + 1].compareTo("万") != 0
								&& as2[i + 1].compareTo("亿") != 0
								&& as2[i + 1].compareTo("元") != 0) {
							as3[k] = as2[i];
							i++;
							k++;
						} else {
			...
			for (k = 0; as3[k] != null; k++) {
				// 将金额数组中的零值替换为"_"
				if (as3[k].compareTo("零") == 0
						&& as3[k + 1] != null
						&& (as3[k + 1].compareTo("万") == 0
								|| as3[k + 1].compareTo("亿") == 0
								|| as3[k + 1].compareTo("元") == 0 || as3[k + 1]
								.compareTo("零") == 0))
					as3[k] = "_";
			}
			...
			for (int j = 0; j < i1; j++) {
				...
				if (as2[j].compareTo("亿") == 0
						&& as2[j + 1].compareTo("万") == 0)
					as2[j + 1] = "";
			}
			if (stringbuffer.indexOf("角") == -1
					&& stringbuffer.indexOf("分") == -1) {
