...
public final class NumberHelper {
	...
	/**
	 * 将银行账号替换空格后格式化为4个数字一组,用连接符分隔
	 * @param account
	 * @param connectstr
	 * @return
	 */
	public static String formatBankAccount(String account, String connectstr){
		return formatBankAccount(account, connectstr, "");
	}
	
	/**
	 * 将银行账号替换连接字符后格式化为4个数字一组,用连接符分隔
	 * @param account
	 * @param connectstr
	 * @return
	 */
	public static String formatBankAccount(String account, String connectstr, String replacestr){
		String r = "";
		...
		char[] ch = account.replaceAll(replacestr, "").toCharArray();
		int i = 0;
		for(char c : ch){
			if((i+1)%5==0){
				r = r + connectstr + c;
