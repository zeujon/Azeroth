...
public class DBSqlUtil {
	...
	public static String getHSQLPagedSql(String sql,int start, int end) {
		return new StringBuffer(sql.length() + 10).append(sql).insert(
				sql.toLowerCase().indexOf("select") + 6, " limit ? ?")
				.toString();
	}
	...
	public static  String getMysqlPagedSql(String sql,int start, int end) {
		return new StringBuffer(sql.length() + 20).append(sql).append(
				" limit ").append(start).append(",").append(end).toString();
	}
	...
	public static  String getSqlServerPageSql(String sqlstr, int spage,
		...
		if (selectitem.toLowerCase().contains("distinct")) {
			...
			selectitem = selectitem
					.substring(selectitem.indexOf("distinct") + 8);
		}

		StringBuffer pagesql = new StringBuffer();
		pagesql.append("select ").append(distin).append(" Top ").append(
				perpagenum).append(" ").append(selectitem).append(" from ")
	 ...
	 * @return
	 */
	public static String getSqlServer2005PageSql_bak(String sqlstr, int spage,
			int perpagenum){
		...
		StringBuffer pagesql = new StringBuffer();
		pagesql.append("select * from (").append(sql1).append(", ROW_NUMBER() OVER (").append(orderstr).append(") as pos")
				.append(sql2).append(") as T where T.pos>").append(spage).append(" and T.pos<=").append(spage+perpagenum);
		return pagesql.toString();
	}
	...
	public static String getSqlServer2005PageSql(String sqlstr, int spage,
			int perpagenum){
		sqlstr=sqlstr.replaceAll("^(\\S*\\()(.*)(\\)\\S*)$", "$2"); //把外面可能多余的括号去掉
		String sql1 = sqlstr.replaceFirst("\\sfrom\\s.*", "");//截取from前面的部分
		String orderstr=sqlstr.replaceAll("(.*)(\\sorder\\s.*)", "$2").replaceAll("^.*(\\sfrom\\s).*$", "order by (select 0)");
		//获取order部分
		String sql2=sqlstr.substring(sqlstr.indexOf(" from ")).replaceAll("\\sorder\\s.*", "");
		StringBuffer pagesql = new StringBuffer();
		pagesql.append("select * from (").append(sql1).append(
				", ROW_NUMBER() OVER (").append(orderstr).append(") as pos ")
				.append(sql2).append(") as T where T.pos>").append(spage)
				.append(" and T.pos<=").append(spage + perpagenum);
		return pagesql.toString();
	}

	// SqlServer数据库直接支持分页语句
	//后加的.这个可以解决复杂的sql分布
	public static  String getSqlServerPageSql(SQLObject sqlobj) {
		...
		sql.append("select ").append(distin).append(" Top ").append(pageSize).append(" ").append(showcolumns)
			.append(" from ").append(tables).append(" where ").append(wherestr).append(" and ")
			.append(pk).append(" NOT IN (select Top ").append(spage).append(" ").append(pk).append(" from ")
			.append(tables).append(" where ").append(wherestr).append(" ")
			.append(groupbystr).append(" ").append(orderbystr).append(") ")
			.append(groupbystr).append(" ").append(orderbystr);
...
//		pagingSelect.append(" ) row_ where rownum <= ?) where rownum_ > ?");
	...
	public static void main(String[] args){
		DBSqlUtil.getSqlServer2005PageSql("select * from docbase", 2, 10);
	}
}
